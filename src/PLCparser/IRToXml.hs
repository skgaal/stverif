...
-- ****************************  Primary functions *************************  --
makePetriNet :: Bool -> (String -> Integer -> Program -> UnknownMap -> String)
makePetriNet editor = if editor then makeEditorNet else makeEngineNet
  where
    makeEditorNet :: String -> Integer -> Program -> UnknownMap -> String
    makeEditorNet _ _ [] _ = []
    makeEditorNet name maxt p m = "<?xml version='1.0' encoding='UTF-8' standalone='no'?> <pnml> <constant name='t' value='"++show maxt++"'/><net active='true' id='"++name++"' type='P/T net'>" ++
                 (serializeWith showeditor . nub $ findAllPlaces p m) ++
                 (serializeWith showeditor $ findAllTransitions p m)  ++
                 (serializeWith showeditor $ findAllArcs p m)         ++
    ...
    makeEngineNet :: String -> Integer -> Program -> UnknownMap -> String
    makeEngineNet _ _ [] _ = []
    makeEngineNet name maxt p m = "<pnml><net id='ComposedModel' type='P/T net'>" ++
                 (serializeWith ((flip showengine) (name++"_")) . nub $ findAllPlaces p m) ++
                 (serializeWith ((flip showengine) (name++"_")) $ findAllTransitions p m)  ++
                 (serializeWith ((flip showengine) (name++"_")) $ findAllArcs p m)         ++
                 generateWatchdogEngine (name++"_") ++
                  "</net></pnml>"

-- ****************************  Finding things ****************************  --
findAllPlaces :: Program -> UnknownMap -> [Place]
findAllPlaces [] _ = []
findAllPlaces ((ProgLine l ins gl):xs) m = findPlacesInstruction l ins m ++
                                           findPlacesGotoLabel gl ++
                                           findAllPlaces xs m
  where
    findPlacesInstruction :: Label -> Instruction -> UnknownMap -> [Place]
    findPlacesInstruction l ins@(Eval _ _ _)      _ = [(Place l 0 $ addTI (Ti 3) $ invariant ins)]
    findPlacesInstruction l ins@(Compare _ _ _ _) _ = [(Place l 0 $ addTI (Ti 3) $ invariant ins)]
    findPlacesInstruction l ins@(If _ il)         _ = [(Place l 0 $ invariant ins)]
    findPlacesInstruction l ins@(Case _ _)        _ = [(Place l 0 $ invariant ins)]
    findPlacesInstruction l ins@(Loop n _ ll le)  _ = [(Place l 0 $ invariant ins)
                                                      ,(Place (l++"p")   n (Tinf 0))
                                                      ,(Place (l++"pp")  0 (Tinf 0))
                                                      ...
                                                      ,(Place ll         0 (TI 0 0))
                                                      ,(Place le         0 (TI 0 0))]
    findPlacesInstruction l ins@(Call _ le)       _ = [(Place l          0 $ invariant ins)
                                                      ,(Place (l++"p")   0 (Tinf 0))]
    findPlacesInstruction l ins@(Unknown i)       m = [(Place l          0 (TI 0 0))] -- TODO: Lookup the unknown in the map!
    findPlacesGotoLabel :: Label -> [Place]
    findPlacesGotoLabel (':':gl) = [(Place gl 0 $ time Return)]
    findPlacesGotoLabel _ = []

findAllTransitions :: Program -> UnknownMap -> [Transition]
findAllTransitions [] _ = []
findAllTransitions (x:xs) m = (findTransitions x m) ++ findAllTransitions xs m
  where
    findTransitions :: ProgLine -> UnknownMap -> [Transition]
    findTransitions (ProgLine l ins@(Eval _ _ _) _)      _ = [(Trans l)]
    findTransitions (ProgLine l ins@(Compare _ _ c _) _) _ = [(Trans l)]
    findTransitions (ProgLine l ins@(If _ _) _)          _ = [(Trans $ l ++ "T")
                                                             ,(Trans $ l ++ "F")]
    findTransitions (ProgLine l ins@(Call _ le) _)       _ = [(Trans l)
                                                             ,(Trans $ l ++ removeColon le)]
    findTransitions (ProgLine l ins@(Loop _ ls _ le) _)  _ = [(Trans l)              -- initial transition     (start in report)
    ...
    findTransitions (ProgLine l ins@(Case _ cs) _)       _ = makeCaseTransitions l (nub . snd $ unzip cs)
      where
    ...
    findTransitions (ProgLine l ins@(Unknown i) _)       m = [(Trans l)] -- TODO: Lookup the name
...
findAllArcs :: Program -> UnknownMap -> [Arc]
findAllArcs [] _ = []
findAllArcs (x:xs) m = (findArcs x m) ++ findAllArcs xs m
  where
    findArcs :: ProgLine -> UnknownMap -> [Arc]
    findArcs (ProgLine l ins@(Eval _ op _) gl)     _ = [(ArcToT l        l        1 $ addTI (Ti 3) $ time $ O op)
                                                       ,(ArcToP l        (removeColon gl)       1)]
    findArcs (ProgLine l ins@(Compare _ _ c _) gl) _ = [(ArcToT l        l        1 $ addTI (Ti 3) $ time $ C c)
                                                       ,(ArcToP l        (removeColon gl)       1)]
    findArcs (ProgLine l ins@(If _ ly) gl)         _ = [(ArcToT l        (l++"T") 1 $ time Jump)
                                                       ,(ArcToT l        (l++"F") 1 $ time Jump)
                                                       ,(ArcToP (l++"T") ly 1)
                                                       ,(ArcToP (l++"F") (removeColon gl) 1)]
    findArcs (ProgLine l ins@(Call ls le) gl)      _ = [(ArcToT l        l          1 $ time OCall)
    ...
    findArcs (ProgLine l ins@(Loop n ls le ll) gl) _ = [(ArcToT l          l          1 $ time Jump) 
                                                       ,(ArcToP l          le         1)             
    ...
    findArcs (ProgLine l ins@(Case _ cs) gl)       _ = findCaseArcs l (nub . snd $ unzip cs) (removeColon gl)
      where
    ...
    findArcs (ProgLine l ins@(Unknown i) gl)       m = [(ArcToT l l  1 (TI 0 0))
                                                       ,(ArcToP l (removeColon gl) 1)] -- TODO: Lookup the map!
